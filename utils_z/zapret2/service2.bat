...
+
+set "ARGS="
+for /f "delims=" %%F in ('powershell -NoProfile -Command "Get-ChildItem -LiteralPath ''%ROOT%'' -Filter ''.bat'' | Where-Object { $_.Name -notlike ''service*'' } | Sort-Object Name | Select-Object -First 1 -ExpandProperty Name"') do set "ARGS=%%F"
+if not defined ARGS (
...
+sc create "%SERVICE_NAME%" binPath= "\"%BIN%\%BYPASS_EXE%\" %ARGS%" start= auto DisplayName= "Zapret2" >nul
+sc description "%SERVICE_NAME%" "zapret2 service using winws2" >nul 2>&1
+sc start "%SERVICE_NAME%" >nul 2>&1
+echo Installed %SERVICE_NAME% using %ARGS%
+pause
+exit /b 0
+
+:remove
...
+for %%P in (winws2.exe winws.exe) do tasklist /FI "IMAGENAME eq %%P" | find /I "%%P" >nul && echo Running: %%P
+pause
+exit /b 0
+
+:tests
