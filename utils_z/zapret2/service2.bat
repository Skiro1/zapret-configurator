...
set "BIN_PATH=%~dp0bin\"
set "MISSING_FILES="
for %%F in (winws2.exe cygwin1.dll Monkey64.sys WinDivert.dll) do (
    if not exist "%BIN_PATH%%%F" set "MISSING_FILES=!MISSING_FILES! %%F"
)
if defined MISSING_FILES (
    call :PrintYellow "Missing required winws2 files in bin:!MISSING_FILES!"
)
echo:
...
for %%P in (winws2.exe winws.exe) do (
    ...
    if !errorlevel!==0 if not defined RUNNING_PROC set "RUNNING_PROC=%%P"
)
if defined RUNNING_PROC (
    call :PrintGreen "Bypass (!RUNNING_PROC!) is RUNNING."
) else (
...
for %%P in (winws2.exe winws.exe) do (
    tasklist /FI "IMAGENAME eq %%P" | find /I "%%P" > nul
    if !errorlevel!==0 set "winws_running=0"
)
...
echo.
start "" powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0utils\test_zapret2.ps1"
pause
goto menu
