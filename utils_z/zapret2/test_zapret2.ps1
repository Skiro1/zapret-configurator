...
$zapretProcessNames = @("winws2", "winws")

# Stop winws2
function Stop-Zapret {
    foreach ($procName in $zapretProcessNames) {
...
function Get-ZapretSnapshot {
    $snapshot = @()
    foreach ($procName in $zapretProcessNames) {
        try {
            $snapshot += Get-CimInstance Win32_Process -Filter "Name='$($procName).exe'" |
...
function Restore-ZapretSnapshot {
    param($snapshot)
    ...
    foreach ($procName in $zapretProcessNames) {
        try {
            $current += (Get-CimInstance Win32_Process -Filter "Name='$($procName).exe'").CommandLine
        } catch {
        }
    }
...
$originalZapretProcesses = Get-ZapretSnapshot
...
try {
    ...
    Stop-Zapret
    Restore-ZapretSnapshot -snapshot $originalZapretProcesses
    if ($originalIpsetStatus -ne "any") {
